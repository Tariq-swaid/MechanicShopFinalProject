...
namespace MechanicShop.Application.Common.Fetures.WorkOrders.Queries.GetWorkOders
{
    public class GetWorkOrdersQueryHandler(IAppDbContext context)
     : IRequestHandler<GetWorkOrdersQuery, Result<PaginatedList<WorkOederListItemDto>>>
    {
        private readonly IAppDbContext _context = context;

        public async Task<Result<PaginatedList<WorkOederListItemDto>>> Handle(GetWorkOrdersQuery query, CancellationToken ct)
        {
            ...
            var items = await workOrdersQuery
                  ...
                  .Take(query.PageSize)
                  .Select(wo => new WorkOederListItemDto
                  {
                      WorkOrderId = wo.Id,
                      ...
                      Labor = wo.Labor != null
                        ? wo.Labor.FisrtName + " " + wo.Labor.LastName
                        : null,
                      State = wo.State,
            ...
            return new PaginatedList<WorkOederListItemDto>
            {
                Items = items,
        ...
        private static IQueryable<WorkOrder> ApplySearchTerm(IQueryable<WorkOrder> query, string searchTerm)
        {
            ...
            return query.Where(wo =>
                ...
                (wo.Labor != null && (
                    wo.Labor.FisrtName.ToLower().Contains(normalized) ||
                    wo.Labor.LastName.ToLower().Contains(normalized) ||
                    (wo.Labor.FisrtName + " " + wo.Labor.LastName).ToLower().Contains(normalized)
                )) ||
