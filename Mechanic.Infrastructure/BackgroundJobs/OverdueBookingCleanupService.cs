using Mechanic.Infrastructure.Settings;
...
namespace Mechanic.Infrastructure.BackgroundJobs
{
     ...
     TimeProvider dateTime) : BackgroundService
    {
        ...
        {
            // 1. Wrap the entire loop in a try-catch for the cancellation
            try
            {
                ...
                while (await timer.WaitForNextTickAsync(stoppingToken))
                {
                    ...
                    // 2. Keep your existing inner try-catch for database/business logic errors
                    try
                    {
                        using var scope = _scopeFactory.CreateScope();
                        var db = scope.ServiceProvider.GetRequiredService<IAppDbContext>();

                        var cutoff = _dateTime.GetUtcNow().AddMinutes(-_appSettings.BookingClellationThreaholdMinutes);
            ...
            {
                // 3. Gracefully handle the shutdown signal
                _logger.LogInformation("OverdueBookingCleanupService is stopping because the application is shutting down.");
            }
        }
    }
}
