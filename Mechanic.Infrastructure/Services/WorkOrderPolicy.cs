...
namespace Mechanic.Infrastructure.Services
{
    public class WorkOrderPolicy(IOptions<AppSettings> options, IAppDbContext context) : IWorkOrderPolicy
    {
        ...
        {
            if ((endAt - startAt) < TimeSpan.FromMinutes(_appSettings.MinmumAppointmentDurationInMinutes))
            {
                return Error.Conflict(
                    "WorkOrder_TooShort",
                    $"WorkOrder duration must be at least {_appSettings.MinmumAppointmentDurationInMinutes} minutes.");
            }
