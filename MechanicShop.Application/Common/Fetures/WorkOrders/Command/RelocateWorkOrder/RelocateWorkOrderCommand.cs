...
public record RelocateWorkOrderCommand(Guid WorkOrderId, DateTimeOffset NewStartAt, Spot NewSpot) : IRequest<Result<Updated>>;
    ...
    : IRequestHandler<RelocateWorkOrderCommand, Result<Updated>>
{
    ...
    public async Task<Result<Updated>> Handle(RelocateWorkOrderCommand command, CancellationToken ct)
    {
        ...
        // 🐛 FIX 1: Check command.NewSpot instead of workOrder.Spot
        ...
        {
            _logger.LogError("Spot: {Spot} is not available.", command.NewSpot.ToString());
        ...
        if (await _appointmentValidator.IsVehicleAlreadyScheduled(workOrder.VehicleId, command.NewStartAt, endAt, command.WorkOrderId))
        {
            ...
            return ApplicationError.VehicleSchedulingConflict;
        }
        ...
        // 🐛 FIX 2: Check updateSpotResult instead of updateTimingResult
        ...
        // 🐛 FIX 3: Removed the duplicate AddDomainEvent call here
