...
{
    // this class is responsible for initializing the database and seeding it with default data. It ensures that the database is created, and if it is empty, it populates it with predefined roles, users, customers, vehicles, repair tasks, and work orders.
    // The class also includes error handling to log any issues that occur during initialization or seeding.
        ...
        RoleManager<IdentityRole> roleManager)
    {
        ...
        public async Task TrySeedAsync()
        {
            ...
            if (!_context.Employes.Any())
            {
                ...
                [
                    Employe.Create(Guid.Parse(manager.Id), "Primary", "Manager", Role.Manager).Value,
                Employe.Create(Guid.Parse(labor01.Id), "John", "S.", Role.Labor).Value,
                Employe.Create(Guid.Parse(labor02.Id), "Peter", "R.", Role.Labor).Value,
                Employe.Create(Guid.Parse(labor03.Id), "Kevin", "M.", Role.Labor).Value,
                Employe.Create(Guid.Parse(labor04.Id), "Suzan", "L.", Role.Labor).Value
                ]);
            }
            ...
            if (!_context.Customers.Any())
            {
                List<Vehicle> vehicles = [
                            Vehicle.Create(id: Guid.Parse("61401e63-007b-4b1c-8914-9eb6e9bd95c5"), make: "Toyota", model: "Camry", year: 2020, licespeplate: "ABC123").Value,
                        Vehicle.Create(id: Guid.Parse("13c80914-41ad-4d46-b7bb-60f6c89ad01e"), make: "Honda", model: "Civic", year: 2018, licespeplate: "XYZ456").Value,
                    ];
                ...
                Customer.Create(id: Guid.Parse("73a04dd3-c81a-4a54-9882-ef1017eb192d"), name: "Sarah Peter", phoneNumber: "987654321", email: "sarah.peter@localhost", vehicles: [Vehicle.Create(id: Guid.Parse("a04f329d-0f5a-46a0-beae-699c034ae401"), make: "Ford", model: "Focus", year: 2021, licespeplate: "DEF789").Value, Vehicle.Create(id: Guid.Parse("cf60e95b-5752-4c26-aa07-31a34164606c"), make: "Chevrolet", model: "Malibu", year: 2019, licespeplate: "GHI012").Value,]).Value,
            ]);
            }
            ...
            if (!_context.RepairTasks.Any())
            {
                _context.RepairTasks.AddRange([
                 RepairTask.Create(id: Guid.Parse("616aebb1-d515-4b40-8d47-8d5c0b67a313"), name: "Engine Oil Change", laborCost: 50.00m, repairDurationInMinutes: RepairDurationInMinutes.Min60, parts: [Part.Create(Guid.Parse("ec65225c-9066-4a1c-974f-f183c39fdd16"), "Engine Oil",1, 25.00m).Value, Part.Create(Guid.Parse("62ad80e3-2cff-41af-ab40-16fab8db8b38"), "Oil Filter",  1, 10.00m).Value]).Value,
                RepairTask.Create(id: Guid.Parse("4fa0be55-06f6-4616-b086-e1f0c9354cd8"), name: "Brake Replacement", laborCost: 150.00m, repairDurationInMinutes: RepairDurationInMinutes.Min90, parts: [Part.Create(Guid.Parse("86375a12-715e-4aa4-aad9-c0f9ccf44a14"), "Brake Pads",2, 40.00m).Value, Part.Create(Guid.Parse("526d89c3-a971-4ea7-ba15-de6b50b13c21"), "Brake Fluid",  1, 15.00m).Value]).Value,
                RepairTask.Create(id: Guid.Parse("a376b5d1-6b2d-4dd8-883e-d3d1721c1316"), name: "Tire Rotation", laborCost: 30.00m, repairDurationInMinutes: RepairDurationInMinutes.Min45, parts: [Part.Create(Guid.Parse("a46f974e-a198-4098-8a1f-6be6e68ec743"), "Tire Valve",  4, 5.00m).Value]).Value,
                RepairTask.Create(id: Guid.Parse("a770cc6e-0c8b-4ac5-9ee6-6928682bd47e"), name: "Battery Replacement", laborCost: 70.00m, repairDurationInMinutes: RepairDurationInMinutes.Min30, parts: [Part.Create(Guid.Parse("d4fd3255-29dc-4d45-9d87-f58ab98bc28b"), "Car Battery", 1, 120.00m).Value]).Value,
                RepairTask.Create(id: Guid.Parse("e4c2b675-4a60-488f-a7b4-61966e7e80e3"), name: "Wheel Alignment", laborCost: 80.00m, repairDurationInMinutes: RepairDurationInMinutes.Min60, parts: [Part.Create(Guid.Parse("fa3b9a7e-1c2d-4e3f-9b8a-0c1d2e3f4a5b"), "Alignment Shim Kit (per wheel)",  4,5.00m).Value]).Value,
                RepairTask.Create(id: Guid.Parse("1cb1608c-3bc7-4325-99c3-8244c0fb412f"), name: "Air Conditioning Recharge", laborCost: 100.00m, repairDurationInMinutes: RepairDurationInMinutes.Min30, parts: [Part.Create(Guid.Parse("526dca0a-d236-47d3-8e8f-c83d555b2de9"), "Refrigerant", 1,50.00m).Value]).Value,
                RepairTask.Create(id: Guid.Parse("a8e9b4e0-8581-40df-967d-51a0f4fabc0e"), name: "Spark Plug Replacement", laborCost: 90.00m, repairDurationInMinutes: RepairDurationInMinutes.Min60, parts: [Part.Create(Guid.Parse("019f5eab-a8a5-44d4-92b3-1f998e3f10c2"), "Spark Plug", 4, 10.00m).Value]).Value,
                RepairTask.Create(id: Guid.Parse("90f2f3ef-3357-439e-9689-628aa08200c1"), name: "Engine Diagnostic", laborCost: 120.00m, repairDurationInMinutes: RepairDurationInMinutes.Min120, parts: [Part.Create(Guid.Parse("c3d4e5f6-a7b8-9c0d-1e2f-3a4b5c6d7e8f"), "Smoke Leak Detector Fluid Cartridge",  1, 20.00m).Value]).Value,
                RepairTask.Create(id: Guid.Parse("d124651e-ca72-467e-ba28-81ea4a2080bc"), name: "Timing Belt Replacement", laborCost: 200.00m, repairDurationInMinutes: RepairDurationInMinutes.Min120, parts: [Part.Create(Guid.Parse("06b764a0-73a2-4c37-b279-adae3856499c"), "Timing Belt",  1, 75.00m).Value]).Value,
                RepairTask.Create(id: Guid.Parse("cee9b309-8620-4028-8d38-2532771ab3ea"), name: "Transmission Fluid Change", laborCost: 100.00m, repairDurationInMinutes: RepairDurationInMinutes.Min45, parts: [Part.Create(Guid.Parse("0a8b0c19-873a-4da0-811b-45ff85bca0ed"), "Transmission Fluid", 1, 60.00m).Value]).Value
                ]);
            }
            ...
            if (!_context.WorkOrders.Any())
            {
                ...
                while (startDate < endDate)
                {
                    foreach (Spot spot in spots)
                    {
                        ...
                        while (occupiedMinutes < minOccupancy && currentTime.TimeOfDay < closeTime)
                        {
                            ...
                            var duration = selectedTask.Sum(st => (int)st.RepairDurationInMinutes);
                ...
                var workOrderStartingNow = WorkOrder.Create(
                    ...
                    startTimeFirstOrder,
                    startTimeFirstOrder.AddMinutes(repairTasksForFirstOrder.Sum(rt => (int)rt.RepairDurationInMinutes)),
          ...
          .First(rt => rt.RepairDurationInMinutes == RepairDurationInMinutes.Min60);
                ...
                var endTimeSecondOrder = roundedStart.AddMinutes((int)repairTasksEndingNow.RepairDurationInMinutes);
